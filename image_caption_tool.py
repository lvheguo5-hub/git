...
class ImageCaptionGenerator:
    ...
    def __init__(self):
        ...
        # 当前设置
        self.current_model = 'Hugging Face BLIP (免费)'
    ...
    async def generate_caption(self, image_path: str, image_data: bytes = None) -> ImageDescription:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if model_type == 'huggingface':
                    return await self._generate_with_huggingface(image_path, image_data)
                elif model_type == 'groq':
                    return await self._generate_with_groq(image_path, image_data)
                else:
    ...
    async def _generate_with_huggingface(self, image_path: str, image_data: bytes) -> ImageDescription:
        """使用Hugging Face API生成描述 - 新增：支持公开免费模型"""
        model_config = self.get_model_config()
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(API_URL, headers=headers, data=payload, files=files) as response:
                if response.status == 200:
                    ...
                    # 应用风格
                    if style_prompt and description:
                    ...
                    raise Exception(f"Hugging Face API错误: {response.status} - {error_text}")
    
    async def _generate_with_groq(self, image_path: str, image_data: bytes) -> ImageDescription:
        """使用Groq API生成描述 - 优化：图片编码为base64"""
        import base64
...
class ImageCaptionApp:
    ...
    def on_model_change(self, event=None):
        ...
        # 检查API密钥
        if model_config.get('api_key') is not None and not model_config.get('api_key'):
            messagebox.showwarning("API密钥缺失", 
                                 f"模型 {self.generator.current_model} 需要API密钥，请在代码中配置。")
    ...
    def generate_descriptions(self):
        ...
        # 在新线程中运行异步任务
        threading.Thread(target=self._generate_async, daemon=True).start()
        ...
        async def generate():
            ...
            for i, image_path in enumerate(self.image_files):
                try:
                    ...
                    # 生成描述
                    description = await self.generator.generate_caption(image_path)
